// A SqlStorage stand-in backed by node:sqlite, so the store/auth modules can
// be tested against real SQL instead of a hand-rolled mock. Only the surface
// the modules use is implemented: exec(query, ...bindings) → { toArray() }.
//
