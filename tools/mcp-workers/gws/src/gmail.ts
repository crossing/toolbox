// Gmail tool surface. Read tools need only gmail.readonly; write tools are
// registered solely for write-scope grants (which also requested the
// compose/modify/labels/settings.basic upstream scopes at consent).

import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { z } from "zod";
import { GoogleApiError, type GoogleClient } from "./api";
import { asResult, DESTRUCTIVE, needsConfirm, READ_ONLY, run, WRITE } from "./toolutil";

const GMAIL = "https://gmail.googleapis.com/gmail/v1/users/me";
const BODY_CHAR_CAP = 20000;
const ATTACHMENT_BYTE_CAP = 1_000_000;

type GetClient = () => Promise<GoogleClient> | GoogleClient;
...
export function registerGmailReadTools(server: McpServer, getClient: GetClient): void {
  ...
  server.registerTool(
    "gmail_list_labels",
    { description: "List Gmail labels (system and user).", inputSchema: {}, annotations: READ_ONLY },
    async () => run(async () => (await getClient()).getJson(`${GMAIL}/labels`)),
  );
  ...
  server.registerTool(
    "gmail_list_drafts",
    {
      ...
      inputSchema: { max_results: z.number().int().min(1).max(50).optional().describe("Default 20") },
      annotations: READ_ONLY,
    },
    async ({ max_results }) =>
      run(async () => (await getClient()).getJson(`${GMAIL}/drafts`, { maxResults: max_results ?? 20 })),
  );
...
export function registerGmailWriteTools(server: McpServer, getClient: GetClient): void {
  server.registerTool(
    ...
    async ({ to, cc, bcc, subject, body }) =>
      run(async () => {
        ...
        return (await getClient()).sendJson("POST", `${GMAIL}/drafts`, { message: { raw } });
      }),
  );
  ...
  server.registerTool(
    ...
    async ({ message_id, add_label_ids, remove_label_ids }) =>
      run(async () =>
        (await getClient()).sendJson("POST", `${GMAIL}/messages/${message_id}/modify`, {
  ...
  server.registerTool(
    "gmail_create_label",
    { description: "Create a Gmail label.", inputSchema: { name: z.string() }, annotations: WRITE },
    async ({ name }) => run(async () => (await getClient()).sendJson("POST", `${GMAIL}/labels`, { name })),
  );
  ...
  server.registerTool(
    "gmail_update_label",
    {
      ...
      inputSchema: { label_id: z.string(), name: z.string() },
      annotations: WRITE,
    },
    async ({ label_id, name }) =>
      run(async () => (await getClient()).sendJson("PATCH", `${GMAIL}/labels/${label_id}`, { name })),
  );
  ...
  server.registerTool(
    "gmail_delete_label",
    {
      ...
      inputSchema: { label_id: z.string(), confirm: z.boolean().optional() },
      annotations: DESTRUCTIVE,
    },
    async ({ label_id, confirm }) => {
      if (confirm !== true) return needsConfirm();
      return run(async () => {
        await (await getClient()).delete(`${GMAIL}/labels/${label_id}`);
  ...
  server.registerTool(
    "gmail_list_filters",
    { description: "List Gmail filters.", inputSchema: {}, annotations: READ_ONLY },
    async () => run(async () => (await getClient()).getJson(`${GMAIL}/settings/filters`)),
  );
  ...
  server.registerTool(
    ...
    async ({ from, to, subject, query, add_label_ids, remove_label_ids }) =>
      run(async () =>
        (await getClient()).sendJson("POST", `${GMAIL}/settings/filters`, {
  ...
  server.registerTool(
    "gmail_delete_filter",
    {
      ...
      inputSchema: { filter_id: z.string(), confirm: z.boolean().optional() },
      annotations: DESTRUCTIVE,
    },
    async ({ filter_id, confirm }) => {
      if (confirm !== true) return needsConfirm();
      return run(async () => {
        await (await getClient()).delete(`${GMAIL}/settings/filters/${filter_id}`);
