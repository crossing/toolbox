// Drive tool surface. Reads need only drive.readonly. Writes need the full
// drive scope (drive.file would only cover files this app created).
//
...
export function registerDriveReadTools(server: McpServer, getClient: GetClient): void {
  server.registerTool(
    ...
    async ({ query, max_results, order_by }) =>
      run(async () =>
        (await getClient()).getJson(`${DRIVE}/files`, {
          q: query,
  ...
  server.registerTool(
    ...
    async ({ file_id }) =>
      run(async () => (await getClient()).getJson(`${DRIVE}/files/${file_id}`, { fields: FILE_FIELDS })),
  );
...
export function registerDriveWriteTools(server: McpServer, getClient: GetClient): void {
  server.registerTool(
    ...
    async ({ name, parent_id, mime_type, content }) =>
      run(async () => {
        const client = await getClient();
  ...
  server.registerTool(
    ...
    async ({ file_id, name, description, add_parent_id, remove_parent_id, content }) =>
      run(async () => {
        const client = await getClient();
  ...
  server.registerTool(
    "drive_trash_file",
    {
      ...
      inputSchema: { file_id: z.string(), confirm: z.boolean().optional() },
      annotations: DESTRUCTIVE,
    },
    async ({ file_id, confirm }) => {
      if (confirm !== true) return needsConfirm();
      return run(async () =>
        (await getClient()).sendJson("PATCH", `${DRIVE}/files/${file_id}`, { trashed: true }, { fields: FILE_FIELDS }),
      );
    },
  );
}
